...
def create_map(dungeon, legend):
    ...
    if TILE_OPACITY < 255:
        np_dungeon[:, :, 3][
            np_dungeon[:, :, 0] == 131] = TILE_OPACITY  # this currently changes the tile background alpha value
...
class Maps(commands.Cog):
    ...
    async def make_map(self, ctx, size: app_commands.Range[int, 5, 100] = None,
                       events: int = 0, seed: str = None, event_seed: str = None):  # , *events):
        events = bool(events)
        if seed is None:
            seed = random.randrange(MAXSIZE)
        ...
        if size is None:
            ...
            size = sorted((5, size, 100))[1]
            random.randrange(10, 40)  # cycle rand so it wont be out of sync

        # newEvents = separateEvents(events)
        dungeonMap = form_map(size)
        # TODO: implement the event pictures here?
        if events:
            if event_seed: random.seed(int(event_seed))
        ...
        class GenerationView(discord.ui.View):
            ...
            if events:
                ...
                async def degenerate(self, inter: discord.Interaction, button: discord.ui.Button):
                    ...
                    await Maps.make_map(tmp_context, ctx=ctx, size=size, events=0, seed=seed)

            @discord.ui.button(label=f"{'Reg' if events else 'G'}enerate Events", style=discord.ButtonStyle.green,
                               custom_id="generate")
            async def generate(self, inter: discord.Interaction, button: discord.ui.Button):
                await inter.response.defer()  # why is this required
                await inter.edit_original_response(view=None)
                await Maps.make_map(tmp_context, ctx=ctx, size=size, events=1,
                                    seed=seed, event_seed=random.randrange(MAXSIZE) if events else None)

            @discord.ui.button(label="Check Seed", style=discord.ButtonStyle.grey, custom_id="seed")
            async def seed(self, inter: discord.Interaction, button: discord.ui.Button):
                await inter.response.defer()
                msg = f"Map seed: {seed}"
                if event_seed and seed != event_seed:
